		...
		__u64 start, __u64 length)
{
	...
	while (*p) {
		...
		if (fsm_intersects(cur, start, (u32)length)) {
			vdfs4_debug_put_err(fsm->sbi, 
					vdfs4_debug_err_bitmap_free,
					0, 0, "freeing already free");
			vdfs4_debug_print_volume_verification(fsm->sbi);
	...
	list_for_each_entry_safe(node, next, &fsm->next_free_list, list) {
		...
		while (*p) {
			...
			if (fsm_intersects(cur, node->start, node->length)) {
				vdfs4_debug_put_err(fsm->sbi, vdfs4_debug_err_bitmap_free,
						0, 0, "freeing already free");
				vdfs4_debug_print_volume_verification(fsm->sbi);
		...
		__u32 *length_in_blocks, unsigned int min_blk_count, int fsm_flags)
{
	...
	if (fsm_flags & VDFS4_FSM_ALLOC_ALIGNED) {
		...
		VDFS4_BUG_ON((length < minimal_length) || (length > optimal_length), sbi);
...
static void fsm_cut_next_free(struct vdfs4_fsm_info *fsm)
{
	...
	list_for_each_entry(next, &fsm->next_free_list, list) {
		...
		VDFS4_BUG_ON(fsm_max_length(node, next->start) < next->length, fsm->sbi);
