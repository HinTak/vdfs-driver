...
#define VDFS4_NUM_OF_SUPPORTED_LAYOUTS \
	(sizeof(supported_layouts)/sizeof(supported_layouts[0]))
...
int vdfs4_sync_super(struct vdfs4_sb_info *sbi, int no)
{
	int ret = 0;
	__u32 checksum;
	...
	if (no == 0)
		offset = VDFS4_EXSB_OFFSET;
	else
		offset = VDFS4_EXSB_COPY_OFFSET;

	set_page_writeback(sbi->superblocks);
	ret = vdfs4_write_page(sbi, sbi->superblocks, offset,
...
static void vdfs4_delayed_prof_write(struct work_struct *work)
{
	...
	if (down_read_trylock(&sb->s_umount)) {
		if (IS_ERR(sbi->prof_file)) {
			...
			sbi->prof_file = filp_open((const char *)path, O_CREAT | O_WRONLY | O_TRUNC,
					S_IRWXU);
			if (IS_ERR(sbi->prof_file))
		...
		if (!IS_ERR(sbi->prof_file)) {
			...
			do {
				size = kfifo_out(&sbi->prof_fifo, &prof_data, sizeof(prof_data));
				if (size) {
					mm_segment_t fs = get_fs();
					set_fs(KERNEL_DS);
					pos = sbi->prof_file->f_path.dentry->d_inode->i_size;
					written = vfs_write(sbi->prof_file, (char*)&prof_data, size, &pos);
					if (written < 0)
...
static void vdfs4_evict_inode(struct inode *inode)
{
	...
	if (error) {
		vdfs4_debug_put_err(sbi, vdfs4_debug_err_inode_evict,
			inode->i_ino, 0, "fail xattrs ino");
	...
	if (S_ISREG(inode->i_mode) || S_ISLNK(inode->i_mode)) {
		...
		if (error) {
			vdfs4_debug_put_err(sbi, vdfs4_debug_err_inode_evict,
					inode->i_ino, 0, "fail truncate ino");
			vdfs4_debug_print_volume_verification(sbi);
			vdfs4_fatal_error(sbi,
	...
	if (error) {
		vdfs4_debug_put_err(sbi, vdfs4_debug_err_inode_evict,
				inode->i_ino, 0, "fail cattree ino");
		vdfs4_debug_print_volume_verification(sbi);
...
static int vdfs4_sb_read(struct super_block *sb, int silent)
{
	...
	if (esb->encryption_flags & (1 << VDFS4_VOLUME_ENCRYPTED))
		set_sbi_flag(sbi, VOLUME_ENCRYPTED);
...
/**
 * @brief		Checks if mounting encrypted image is possible
 * @param [in]	sbi	The vdfs4 superblock info
 * @return		Returns 0 on success, errno on failure
 */
static int vdfs4_encryption_checks(struct vdfs4_sb_info *sbi)
{
	if(!VDFS4_IS_VOL_ENCRYPTED(sbi))
		return 0;

#ifndef CONFIG_VDFS4_DECRYPTION_SUPPORT
	VDFS4_MOUNT_INFO("Volume seems to be encrypted but driver"
			"doesn't support decryption.\n");
	return -EPERM;
#endif
	VDFS4_MOUNT_INFO("Mounting encrypted volume\n");

/* TODO encryption:
 * if CONFIG_VDFS4_HW2_DECRYPTION then we must check in
 * get_hw_capability() if is AES is supported
 */
#ifdef CONFIG_VDFS4_SW_DECRYPTION
	vdfs4_crypto_alloc_workaround();
#endif
	return 0;
}
...
static int vdfs4_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	//print vdfs volume verification result
	vdfs4_debug_print_volume_verification(sbi);
	...
	ret = vdfs4_encryption_checks(sbi);
	if(ret)
	{
		VDFS4_ERR("encryption check error (%s,ret:%d)\n", 
					bdevname(sb->s_bdev, bdev_name), ret);
		goto vdfs4_encryption_error;
	}
...
#ifdef CONFIG_VDFS4_SQUEEZE_PROFILING
	if (!(VDFS4_IS_READONLY(sb)) &&
		(!strncmp(sbi->sb->s_id, CONFIG_VDFS4_SQUEEZE_PROFILING_PARTITION,
