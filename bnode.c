		...
		const char *fmt, ...)
{
	...
	if (err_type) {
		vdfs4_debug_put_err(sbi, err_type, bnode->node_id, 0, fmt);
		...
		int force_insert)
{
	...
	page_idx = (unsigned long)bnode->node_id * (unsigned long)btree->pages_per_node;
...
do_reread:
	...
	if (!create && bnode->node_id != 0 &&
		...
		VDFS4_ERR("Wrong bnode magic(%s,type:%u,mode:%u):"
				...
				btree->sbi->sb->s_id, btree->btree_type, bnode->mode,
				*(u16*)data, data, btree->inode->i_ino, bnode->node_id);
	...
	if (!create && !PageDirty(bnode->pages[0]) &&
		...
		if (real_version != version) {
			VDFS4_ERR("METADATA PAGE VERSION MISMATCH(%s, type:%u,mode:%u):"
					"inode_num-%lu,bnode_num-%u,addr:0x%p,"
					" must be - %u.%u,"
					" real - %u.%u",
					btree->sbi->sb->s_id, btree->btree_type, bnode->mode,
					btree->inode->i_ino, bnode->node_id, data,
					VDFS4_MOUNT_COUNT(version), VDFS4_SYNC_COUNT(version),
					VDFS4_MOUNT_COUNT(real_version), VDFS4_SYNC_COUNT(real_version));
			goto err_exit_vunmap;
		}
	}
#if defined(CONFIG_VDFS4_CRC_CHECK)
	...
	if ((!create) && (!PageChecked(bnode->pages[0]) &&
		...
		if (from_disk != calculated) {
			VDFS4_ERR("CRC mismatch(%s,type:%u,mode:%u):"
...
err_exit_vunmap:
	...
	vdfs4_debug_put_err(btree->sbi, vdfs4_debug_err_bnode_read,
		 btree->btree_type, page_idx,"bnode read err");
	vdfs4_debug_print_volume_verification(btree->sbi);
#ifdef CONFIG_VDFS4_DEBUG
	__vdfs4_dump_bnode_to_console(bnode, data);
	{
		...
		unsigned int base_table_length = VDFS4_LAST_TABLE_INDEX(sbi,
			VDFS4_LSFILE) * sizeof(struct vdfs4_base_table_record) +
			sizeof(struct vdfs4_base_table);

		VDFS4_ERR("dump base table");
		dump_bnode(base_t, &pg_null, 0, base_table_length);
...
struct vdfs4_bnode *vdfs4_alloc_new_bnode(struct vdfs4_btree *btree)
{
	...
	bitmap->first_free_id = (__u32)find_next_zero_bit(bitmap->data,
			(unsigned long)bitmap->bits_num, (unsigned long)bitmap->first_free_id);
	bitmap->free_num--;
