		...
		int comp_extent_idx)
{
	...
	VDFS4_BUG_ON(comp_extent_idx >= inode_i->fbc->comp_extents_n, VDFS4_SB(inode->i_sb));
...
again:
	...
	for (; count < pages_count; count++) {
		if ((unsigned) bio_add_page(bio, page[count],
			...
			if (bio->bi_vcnt) {
				continue_load = 1;
				sector_addr += ((sector_t)count << (PAGE_CACHE_SHIFT -
						SECTOR_SIZE_SHIFT));
			} else {
		...
		unsigned int magic_len)
{
	...
	if (memcmp(VDFS4_CRC32_OFFSET((char *)buff, buff_size), &crc,
		...
		VDFS4_ERR("index:%lu phy addr: 0x%llx", page->index,
				...
				page_to_phys(page));
		vdfs4_debug_put_err(sbi, vdfs4_debug_err_bitmap_validate,
				*((unsigned int*)VDFS4_CRC32_OFFSET((char *)buff, buff_size)),
				crc, "bitmap validate err");
		vdfs4_debug_print_volume_verification(sbi);

#ifdef CONFIG_VDFS4_DEBUG
		mutex_lock(&sbi->dump_meta);
		VDFS4_PRE_DUMP();
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS,
				16, 1, buff, buff_size, 1);
		VDFS4_POST_DUMP();
...
#ifdef CONFIG_VDFS4_UMOUNT_STATISTIC
		...
		pgoff_t chunk_idx, struct vdfs4_comp_extent_info *cext)
{
	...
	if (!test_bit(VDFS4_PROFILED_FILE, &inode_i->flags) &&
		...
		VDFS4_PRE_DUMP();
		VDFS4_ERR("Wrong magic in compressed extent: #%ld, chunk idx:%lu, ext offset:%lld, addr:0x%x(0x%02X 0x%02X, %s)",
			   inode_i->vfs_inode.i_ino, chunk_idx, (long long)extent_offset,
			   (unsigned int)(raw_extent->magic), raw_extent->magic[0],raw_extent->magic[1],
			   VDFS4_COMPR_EXT_MAGIC);
		print_hex_dump( KERN_ERR, "" , DUMP_PREFIX_ADDRESS, 
							16, 1, data, 4096, 1);
		vdfs4_debug_print_volume_verification(VDFS4_SB(inode_i->vfs_inode.i_sb));
		VDFS4_POST_DUMP();
		ret = -EINVAL;
		goto out_unmap;
	}
	...
	real_len = (cext->flags & VDFS4_CHUNK_FLAG_ENCRYPTED) ?
			ALIGN(cext->len_bytes, VDFS4_AES_CHUNK_ALIGN_LEN) :
			cext->len_bytes;
	last_block = (pgoff_t)(start + real_len + PAGE_CACHE_SIZE - 1)
					>> PAGE_CACHE_SHIFT;
...
out_unmap:
	if (!ret && (cext->len_bytes < 0 || cext->offset < 0)) {
		VDFS4_ERR("Invalid compressed extent: #%ld, chunk idx:%lu, ext offset:%lld, cext len:%d, cext offset:%d",
			   inode_i->vfs_inode.i_ino, chunk_idx, (long long)extent_offset, cext->len_bytes, cext->offset);
		VDFS4_PRE_DUMP();
		print_hex_dump( KERN_ERR, "" , DUMP_PREFIX_ADDRESS, 
							16, 1, data, 4096, 1);
		vdfs4_debug_print_volume_verification(VDFS4_SB(inode_i->vfs_inode.i_sb));
...
static inline int is_hw_decr_supported(struct vdfs4_inode_info* inode_i,
		const struct hw_capability* hw_cap)
{
	int encrypted = test_bit(VDFS4_ENCRYPTED_FILE, &inode_i->flags);
	if(!encrypted)
		return 1;
	else
#ifdef CONFIG_VDFS4_HW_DECRYPTION
		return (int)(hw_cap->enc_type & HW_IOVEC_ENCRYPT_AES128_CTR);
#else
		return 0;
#endif
}
...
void *vdfs_get_hwdec_fn(struct vdfs4_inode_info *inode_i)
{
	...
	if (inode_i->vfs_inode.i_sb->s_bdev->bd_disk->fops->hw_decompress_vec
		...
		&& is_hw_decr_supported(inode_i, &hw_cap))
		...
		struct req_hw* req)
{
	...
	int encr = is_vdfs4_inode_flag_set(inode, VDFS4_ENCRYPTED_FILE);
	...
	if(!compr)
		req->compr_type = (hw_cap.comp_type & HW_IOVEC_COMP_UNCOMPRESSED) ?
				HW_IOVEC_COMP_UNCOMPRESSED : 0;
	else
		req->compr_type = convert_hw_comptype(VDFS4_I(inode)->fbc->compr_type);
	...
	if(encr) {
#ifdef CONFIG_VDFS4_HW_DECRYPTION
		VDFS4_BUG_ON(sizeof(req->enc_ivec) != AES_IVEC_SIZE, VDFS4_SB(inode->i_sb));
		ret = vdfs4_crypto_hw2_prep_req(req, inode, comp_extent_idx);
		if(ret)
			return ret;
#else
		return -EINVAL;
		...
		req->enc_type = HW_IOVEC_ENCRYPT_NONE;
	...
	if (auth) {
		 ...
		 * It looks like do_decompression() can process every hash type */
			return -EINVAL;
...
#ifdef CONFIG_VDFS4_HW_DECRYPTION
	if(ret == -EBUSY &&
			(cext.flags & VDFS4_CHUNK_FLAG_ENCRYPTED) &&
			vdfs4_crypto_only_hw2_possible()) {
		io_schedule();
		goto retry;
	}
	else
		...
		struct page *page)
{
	...
	void *decrypted_buf = NULL, *compressed_src;
#ifdef CONFIG_VDFS4_SW_DECRYPTION
	struct page **decrypted_pages = NULL;
#endif
	...
	if (cext->flags & VDFS4_CHUNK_FLAG_ENCRYPTED) {
#ifdef CONFIG_VDFS4_SW_DECRYPTION
		/* mkfs must ensure we can align to 16 safely */
		int len = ALIGN(cext->len_bytes, VDFS4_AES_CHUNK_ALIGN_LEN);
		struct page** decrypt_dst = unpacked_pages;
		int decrypt_dst_offset = 0;

		if(!(cext->flags & VDFS4_CHUNK_FLAG_UNCOMPR)) {
			ret = vdfs4_crypto_alloc_buffer(&decrypted_pages,
							cext->blocks_n, &decrypted_buf);
			if(ret)
				goto exit_free_pages;
			decrypt_dst = decrypted_pages;
			decrypt_dst_offset = cext->offset;
		}

		ret = vdfs4_crypto_decrypt_comp_ext(
				chunk_pages, cext->blocks_n, cext->offset, len,
				decrypt_dst, cext->blocks_n, decrypt_dst_offset,
				inode, comp_extent_idx);
		if(ret || (cext->flags & VDFS4_CHUNK_FLAG_UNCOMPR))
			goto exit_free_pages;
#else
		ret = -EINVAL;
		goto exit_free_pages;
	...
	compressed_src = cext->flags & VDFS4_CHUNK_FLAG_ENCRYPTED ?
						decrypted_buf : src;
	ret = inode_i->fbc->decomp_fn(compressed_src,
...
#ifdef CONFIG_VDFS4_SW_DECRYPTION
	if (decrypted_buf)
		vdfs4_crypto_free_buffer(decrypted_pages, cext->blocks_n,
				decrypted_buf);
#endif
exit_alloc_pages:
