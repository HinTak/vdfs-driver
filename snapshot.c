...
static void dump_basetable(void *data, struct page **pages,
...
__u64 validate_base_table(struct vdfs4_base_table *table)
{
	...
	if (is_not_base_table) {
		VDFS4_ERR("This is not base table (%#x)",
	...
	if( checksum != on_disk_checksum ) {
		VDFS4_ERR("basetable crc mismatch.(org:%#x,calc:%#x)",
		...
		sector_t *ext_table_start, __u64 *table_version_out)
{
	...
	if (!checked) {
		...
		vdfs4_debug_put_err(sbi, vdfs4_debug_err_basetable_load,
				(uint32_t)(table1_version&0xFFFFFFFF),
				(uint32_t)(table2_version&0xFFFFFFFF),
				"fail load baseT");
		vdfs4_debug_print_volume_verification(sbi);
		VDFS4_ERR("Failed to validate basetable.");
#ifdef CONFIG_VDFS4_DEBUG
		dump_basetable(table, &pg_null, 0, (long unsigned int)(DIV_ROUND_UP(tables_size_in_bytes,
			(__u64)PAGE_SIZE) << PAGE_SHIFT));
#endif
		vfree(table);
		return -EFAULT;
		...
		ino_t ino_n, unsigned long *iblock)
{
	...
	bitmap_set(snapshot_bitmap, (unsigned int)position, (1 << alignment));
	bitmap_set(next_snapshot_bitmap, (unsigned int)position, (1 << alignment));
	bitmap_set(snapshot->moved_iblock, (unsigned int)position, (1 << alignment));
		...
		pgoff_t page_index)
{
	...
	VDFS4_BUG_ON((object_id < VDFS4_FSFILE) || (object_id > VDFS4_LSFILE), sbi);
		...
		unsigned int page_count)
{
	...
	do {
		...
		VDFS4_BUG_ON(vdfs4_get_meta_iblock(sbi, i_ino, pages[count]->index,
				&meta_iblock), sbi);
		/* page must be moved */
...
#endif
	VDFS4_BUG_ON((table_size > PAGE_SIZE) && (type != SNAPSHOT_BASE_TABLE), sbi);

	if (table_size > PAGE_SIZE && type == SNAPSHOT_BASE_TABLE) {
		s_iblock = iblock + 1;
		ret = vdfs4_table_IO(sbi, (char *)table + PAGE_SIZE,
			table_size - PAGE_SIZE, WRITE_FLUSH_FUA | REQ_META | REQ_PRIO,
			&s_iblock);
		if (ret) {
			up_read(&sbi->snapshot_info->tables_lock);
			return ret;
		...
		__u64 new_table_index, int force_insert)
{
	...
	if ((free_blocks - (long)new_blocks_count) < ((long)meta_tbc >> 1)) {
		...
		if (ret) {
			...
			free_blocks = free_blocks_in_bitmap_contig_check(snapshot_bitmap,
