...
static int vdfs4_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	if (ret) {
		vdfs4_debug_put_err(sbi, vdfs4_debug_err_inode_unlink,
			dir->i_ino, inode->i_ino, "fail update orphan");
		...
		ino_t ino, sector_t iblock, u32 blk_cnt)
{
	...
	kfifo_in_locked(&sbi->prof_fifo, &prof_data, sizeof(prof_data), &sbi->prof_lock);
}
#endif
...
/**
 * @brief                      This is a special get_block_t callback which is used by
 *                             vdfs4_writepage(). This function will be not called in normal.
 * @param [in]         inode           Pointer to inode structure.
 * @param [in]         iblock          Requested logical block number.
 * @param [in, out]    bh_result       Pointer to buffer_head.
 * @param [in]         create          "Expand file allowed" flag.
...
static int get_file_descr_length(struct vdfs4_comp_file_descr *descr)
{
	switch (descr->layout_version) {
		...
		default:
			VDFS4_ERR("Unsupported compressed file layout %04x", descr->layout_version);
			return -1;
	}
}
	...
	if (is_vdfs4_inode_flag_set(inode, VDFS4_PROFILED_FILE)) {
		...
		if (inode_i->sci[cur_chunk_idx].order == VDFS4_COMPR_NON_SQUEEZE)
			...
			list_for_each_safe(ptr, next, &inode_i->to_read_list) {
				entry = list_entry(ptr, struct vdfs4_squeeze_chunk_info, list);
			...
			entry = list_first_entry(&inode_i->to_read_list, struct vdfs4_squeeze_chunk_info, list);
			list_del(&entry->list);
			spin_unlock(&inode_i->to_read_lock);
 ...
 */
static ssize_t vdfs4_direct_IO(struct kiocb *iocb, struct iov_iter *iter, loff_t offset)
{
 ...
 */
static ssize_t vdfs4_file_write_iter(struct kiocb *iocb, struct iov_iter * iov_iter)
{
	ssize_t ret = 0;
 ...
 */
static ssize_t vdfs4_file_read_iter(struct kiocb *iocb, struct iov_iter * iov_iter)
{
#if defined(CONFIG_VDFS4_EXEC_ONLY_AUTHENTICATED) || defined(CONFIG_VDFS4_DEBUG)
		...
		struct vm_area_struct *vma)
{
	...
	if (!is_vdfs4_inode_flag_set(inode, VDFS4_AUTH_FILE)) {
		if (vma->vm_flags & VM_EXEC) {
			...
			if (!VDFS4_I(inode)->informed_about_fail_read) {
				VDFS4_I(inode)->informed_about_fail_read = 1;
				VDFS4_ERR("Try to execute non-auth file %lu:%s [task:%s(%d)]",
						inode->i_ino,
						VDFS4_I(inode)->name,
						current->comm,
						current->pid);
...
static inline int vdfs4_prepare_encrypted_file_inode(struct vdfs4_inode_info *inode_i,
		struct vdfs4_comp_file_descr *descr)
{
#ifdef CONFIG_VDFS4_DECRYPTION_SUPPORT
	struct vdfs4_sb_info *sbi = inode_i->vfs_inode.i_sb->s_fs_info;

	VDFS4_BUG_ON(!VDFS4_IS_VOL_ENCRYPTED(sbi), sbi);
	VDFS4_BUG_ON(inode_i->record_type != VDFS4_CATALOG_FILE_RECORD, sbi);
	VDFS4_BUG_ON(!inode_i->fbc, sbi);

	memcpy(inode_i->fbc->aes_nonce, descr->aes_nonce, VDFS4_AES_NONCE_SIZE);
	...
	return 0;
#else /* ifdef VDFS4_DECRYPTION_SUPPORT */
	return -EINVAL;
#endif
}
...
int vdfs4_prepare_compressed_file_inode(struct vdfs4_inode_info *inode_i)
{
	...
	if(is_vdfs4_inode_flag_set(inode, VDFS4_ENCRYPTED_FILE)) {
		ret = vdfs4_prepare_encrypted_file_inode(inode_i, &descr);
		if(ret) {
			goto ERROR_FBC;
		}
	}
		...
		struct vdfs4_comp_file_descr* descr, void* data)
{
	...
	inode_i->sci = kmalloc(extents_num*sizeof(struct vdfs4_squeeze_chunk_info), GFP_NOFS);
	if (!inode_i->sci)
		return -ENOMEM;
...
int vdfs4_init_file_decompression(struct vdfs4_inode_info *inode_i, int debug)
{
	...
	pages_num = (unsigned int)(((inode_i->fbc->comp_size + PAGE_CACHE_SIZE - 1) >>
	...
	inode_i->fbc->log_chunk_size = (size_t)le32_to_cpu(descr.log_chunk_size);
...
#ifndef CONFIG_VDFS4_DECRYPTION_SUPPORT
	if(is_vdfs4_inode_flag_set(inode, VDFS4_ENCRYPTED_FILE)) {
		/* no means for decryption present */
		ret = -EOPNOTSUPP;
		goto out;
	}
#endif
...
int __vdfs4_write_inode(struct vdfs4_sb_info *sbi, struct inode *inode)
{
	...
	if (is_vdfs4_inode_flag_set(inode, HARD_LINK))
		...
		record = vdfs4_cattree_find_inode(sbi->catalog_tree,
				...
				VDFS4_BNODE_MODE_RW);
	if (IS_ERR(record)) {
		vdfs4_debug_put_err(sbi, vdfs4_debug_err_inode_write,
				inode->i_ino, 0, "fail update inode");
		vdfs4_debug_print_volume_verification(sbi);
